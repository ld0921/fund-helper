...
function computeRebalancePlan(targetPicks, newMoney){
  ...
  allPicks.forEach(pick=>{
    ...
    const targetAmt=pick.amt; // 直接使用pick.amt作为目标金额，避免舍入误差
  ...
  if(Math.abs(actualBuyTotal - newMoney) > 0 && buyActions.length > 0){
...
function _doGenerate(shouldScroll){
  ...
  if(stratEl){
    ...
    stratEl.innerHTML = `
      ...
      去除冗余指标，保留5个正交维度：<br>
      · <b>Calmar Ratio</b>（32%权重）：混合Calmar——短期=(近1年收益−基准)/近3年最大回撤(60%)，长期=(近3年年化收益−基准)/近3年最大回撤(40%)。<b>主动/QDII基金用沪深300近1年收益作为Alpha基准</b>，指数/债券/货币用无风险利率${RISK_FREE}%<br>
