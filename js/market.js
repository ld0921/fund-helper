...
function inferMomentumPhase(catRanks){
  ...
  // phase 参数映射（用于确认期回退）
  const _phaseParams = {
    overheat:    { label:'权益极端强势', equityMult:0.92, bondMult:1.05, desc:'权益涨幅显著高于历史均值且债券走弱，可能存在过热风险。建议控制权益仓位，适度增加债券作为防御对冲。' },
  ...
  if(equityStrong && bondWeak && equitySig > 10){
    ...
    equityMult = 0.92; bondMult = 1.05;
    desc = '权益涨幅显著高于历史均值且债券走弱，可能存在过热风险。建议控制权益仓位，适度增加债券作为防御对冲。';
