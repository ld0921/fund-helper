...
function calcDCAScore(f){
  ...
  if(f.r3 > -20){
    ...
    if(vol > 0){
      ...
      const optimalVol = f.cat==='bond'&&f.maxDD>=25 ? 3.5 : {active:5, index:5, bond:0.8, qdii:7}[f.cat] || 5;
      const volSigma   = f.cat==='bond'&&f.maxDD>=25 ? 2.5 : {active:3, index:3, bond:0.5, qdii:4}[f.cat] || 3;
